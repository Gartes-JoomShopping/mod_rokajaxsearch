...
if(b.key=="enter"){b.stop();}},paste:function(e){
	var self = this;
...
},keyup:function(g){console.log('keyup');if(g.code==17||g.code==18||g.code==224||g.alt||g.control||g.meta){return false;}if(g.alt||g.control||g.meta||g.key=="esc"||g.key=="up"||g.key=="down"||g.key=="left"||g.key=="right"){return true;
...
}else{a.timer=d.get.delay(500,d,[{type:"raw",option:"com_search",view:"search",category_id:document.id("category_id").value,searchphrase:a.options.phrase,ordering:a.options.ordering,limit:a.options.limit,searchword:this.value.replace(/\"/g,""),tmpl:"component",r:Date.now()}]);
